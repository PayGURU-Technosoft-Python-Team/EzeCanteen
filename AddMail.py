...
def send_monthly_report_email(year=None, month=None):
    ...
    try:
        ...
        report_path = reportGen.generate_monthly_report(year, month)
        if not report_path or not os.path.exists(report_path):
...
def check_and_send_scheduled_email():
    ...
    try:
        ...
        if current_time.hour == scheduled_time.hour and current_time.minute == scheduled_time.minute:
            ...
            # Send the email
            result = send_daily_report_email(today)
            ...
            if result:
                ...
                print(f"Scheduled email sent for {today}")
                return True
            else:
                print(f"Failed to send scheduled email for {today}")
                return False
...
class MailSettingsWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        smtp_layout.addRow("SMTP Password:", self.smtp_pass)
