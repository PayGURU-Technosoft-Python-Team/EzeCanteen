...
class EzeeCanteenApp(QMainWindow):
    ...
    def load_settings(self):
        ...
        try:
            if os.path.exists('appSettings.json'):
                ...
                # Load menu type
                menu_type = canteen_menu.get('Type', 'timeOptions')
                if menu_type == 'timeOptions':
                    self.time_based_radio.setChecked(True)
                elif menu_type == 'custom':
                    self.custom_radio.setChecked(True)
                elif menu_type == 'device':
                ...
                # Load punch interval (for custom mode)
                punch_interval = canteen_menu.get('PunchInterval', '1')
                ...
                # Load meal schedule (for time-based mode)
                ...
                # Load food items (for custom mode)
                food_items = canteen_menu.get('FoodItems', [])
                ...
                for food_item in food_items:
                    ...
                    row['price_input'].setText(item_price)
    ...
    def save_changes(self):
        ...
        try:
            ...
            # Determine menu type based on selected radio button
            if self.time_based_radio.isChecked():
                menu_type = 'timeOptions'
            elif self.custom_radio.isChecked():
                menu_type = 'custom'
            elif self.device_radio.isChecked():
                menu_type = 'device'
            else:
                menu_type = 'timeOptions'  # Default
            
            # Update CanteenMenu settings
            settings['CanteenMenu']['Type'] = menu_type
            ...
            if menu_type == 'custom':
                # Save punch interval for custom mode
                settings['CanteenMenu']['PunchInterval'] = self.punch_interval_input.text()
                ...
                settings['CanteenMenu']['FoodItems'] = food_items
                
                # Clear meal schedule for custom mode
                settings['CanteenMenu']['MealSchedule'] = []
                
            elif menu_type == 'timeOptions':
                # Collect meal schedules for time-based mode
                meal_schedule = []
                ...
                # Clear food items for time-based mode
                settings['CanteenMenu']['FoodItems'] = []
                
            elif menu_type == 'device':
                # Clear both for device mode
                settings['CanteenMenu']['MealSchedule'] = []
                settings['CanteenMenu']['FoodItems'] = []
