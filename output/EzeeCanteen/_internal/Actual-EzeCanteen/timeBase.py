...
try:
    if not os.path.exists('appSettings.json'):
        ...
        print("Created default appSettings.json file with basic configuration")
except Exception as e:
    print(f"Error creating default appSettings.json: {e}")
...
class Communicator(QObject):
    ...
    error_signal = pyqtSignal(dict)  # New signal for error messages
...
class AuthEventMonitor(QThread):
    def __init__(self, communicator, device_ip=None):
        ...
        # Initialize meal schedule to empty list by default
        self.meal_schedule = []
        self.from_time_str = ''
        self.to_time_str = ''
        ...
        try:
            ...
            # Ensure meal_schedule is initialized even if the file can't be read
            self.meal_schedule = []
            self.app_settings = {}
    ...
    def check_time_range(self):
        ...
        # Defensive check to make sure meal_schedule exists
        if not hasattr(self, 'meal_schedule') or self.meal_schedule is None:
            self.meal_schedule = []
            return True  # If no meal schedule specified, always proceed
        
        if not self.meal_schedule:
            return True  # If empty meal schedule, always proceed
        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    logging.error(f"Error in check_time_range: {e}")
                    current_in_meal_time = True  # Default to true to continue processing
            ...
            logging.error(f"Critical error in AuthEventMonitor: {e}")
            import traceback
            logging.error(traceback.format_exc())
            # Signal to the main app to display the error
            error_dict = {
                'error': str(e),
                'traceback': traceback.format_exc(),
                'error_type': 'AuthEventMonitor'
            }
            try:
                self.communicator.error_signal.emit(error_dict)
            except Exception:
                # If error_signal doesn't exist, we can't do much
                pass
...
class EzeeCanteen(QMainWindow):
    def __init__(self):
        ...
        self.communicator.error_signal.connect(self.display_error)
    ...
    def display_error(self, error_dict):
        """Display error message in the UI instead of crashing"""
        error_msg = error_dict.get('error', 'Unknown error')
        error_type = error_dict.get('error_type', 'Application Error')
        traceback_text = error_dict.get('traceback', '')
        
        logging.error(f"Displaying error: {error_type} - {error_msg}")
        
        # Create an error display frame
        error_frame = QFrame()
        error_layout = QVBoxLayout(error_frame)
        
        error_title = QLabel(f"Error in {error_type}")
        ...
        error_message = QLabel(f"An error occurred:\n{error_msg}")
        ...
        if traceback_text:
            ...
            details_text = QLabel(traceback_text)
        ...
        """)
        dismiss_button.clicked.connect(lambda: self.dismiss_error(error_frame))
        error_layout.addWidget(dismiss_button, alignment=Qt.AlignCenter)
        
        # Add the error frame to the UI
        # Find a good place to show it - we'll overlay it on the grid
        if hasattr(self, 'grid_widget') and not sip.isdeleted(self.grid_widget):
            ...
            # Position error frame in the center
            error_frame.setParent(overlay)
            error_frame.setStyleSheet("""
                ...
                padding: 20px;
            """)
            error_frame.setFixedWidth(600)
            
            # Center the error frame
            error_frame.move(
                (overlay.width() - error_frame.width()) // 2,
                (overlay.height() - error_frame.height()) // 2
            )
            error_frame.show()
            
            # Store references to these widgets to remove them later
            self.error_overlay = overlay
            self.error_frame = error_frame
        else:
            # If grid widget isn't available, use a message box as fallback
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.critical(self, f"Error in {error_type}", f"{error_msg}\n\n{traceback_text}")
    
    def dismiss_error(self, error_frame):
        """Dismiss the error message overlay"""
        if hasattr(self, 'error_overlay'):
            self.error_overlay.deleteLater()
            delattr(self, 'error_overlay')
        if hasattr(self, 'error_frame'):
            self.error_frame.deleteLater()
            delattr(self, 'error_frame')
        ...
        for device_ip, config in DEVICES.items():
            try:
                ...
                try:
                    ...
                    logging.error(f"Error starting monitor for device {device_ip}: {monitor_err}")
                    # Create a basic error display
                    error_dict = {
                        'error': str(monitor_err),
                        'error_type': 'Device Monitor',
                        'traceback': traceback.format_exc()
                    }
                    # Schedule error display after the UI is shown
                    QTimer.singleShot(1000, lambda: self.display_error(error_dict))
    ...
    def setup_single_device_monitor(self):
        ...
        # Initialize default header/footer settings
        ...
        try:
            ...
            logging.info("Started authentication event monitor thread")
        except Exception as e:
            logging.error(f"Error starting authentication event monitor: {e}")
            import traceback
            error_dict = {
                'error': str(e),
                'error_type': 'Authentication Monitor',
                'traceback': traceback.format_exc()
            }
            # Schedule error display after the UI is shown
            QTimer.singleShot(1000, lambda: self.display_error(error_dict))
        ...
        try:
            ...
            # No need to set defaults here as they're already set above
    ...
    def add_auth_event(self, event_data):
        ...
        if(event_data.get('employeeNoString')):
            ...
            try:
                ...
                logging.error("appSettings.json file not found for DisablePunch feature")
            except Exception as e:
            ...
            try:
                ...
                # Set default values when appSettings.json doesn't exist
                logging.error("appSettings.json file not found for meal schedule")
                coupon_type = "MEAL"
                if not hasattr(self, 'header'):
                    self.header = {'enable': True, 'text': "EzeeCanteen"}
                if not hasattr(self, 'footer'):
                    self.footer = {'enable': True, 'text': "Thank you!"}
                if not hasattr(self, 'special_message'):
                    self.special_message = ""
            except Exception as e:
            ...
            if "AttendanceInfo" in event_data:
                ...
                if status:
                    ...
                    try:
                        ...
                        logging.error("appSettings.json file not found for special message")
                        if not hasattr(self, 'special_message'):
...
def main():
    ...
    # Check if appSettings.json exists, and create a default one if it doesn't
    if not os.path.exists('appSettings.json'):
        try:
            # Create default settings
            default_settings = {
            ...
            logging.info("Created default appSettings.json file")
            print("Created default appSettings.json file with basic configuration")
        except Exception as e:
            logging.error(f"Error creating default appSettings.json: {e}")
            print(f"Error creating default appSettings.json: {e}")
    ...
    if standalone:
        ...
        def exception_hook(exctype, value, traceback):
            # Log the exception
            logging.error(f"Uncaught exception: {exctype.__name__}: {value}")
            logging.error(f"Traceback: {traceback}")
            
            # Format traceback for display
            import traceback as tb
            error_text = ''.join(tb.format_exception(exctype, value, traceback))
            ...
            error_dialog.setText("An unhandled error occurred:")
